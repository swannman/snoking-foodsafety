...
// null (no inspections on record) deliberately renders as 0 — in this view "nothing found" and
// "nothing on file" read the same. D1 and the MCP API keep the null so the distinction isn't lost.
function mjColor(d){var v=d.mj==null?0:Math.max(0,d.mj);
...
function buildMetrics(){
  ...
  majpts:{label:"Major violations (avg pts)",min:0,max:MJMAX,step:5,val:function(d){return d.mj==null?0:d.mj;},fmt:function(a,b){return a+" – "+(b>=MJMAX?b+"+":b)+" pts";},disp:function(d){return (d.mj==null?0:d.mj)+" pts major avg";}},
